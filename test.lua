-- ОЧИСТКА ДУБЛИКАТОВ ИНТЕРФЕЙСА ПРИ ПЕРЕЗАПУСКЕ
...
RunService.Stepped:Connect(function() if noclip and char then for _, part in pairs(char:GetDescendants()) do if part:IsA("BasePart") then part.CanCollide = false end end end end)
