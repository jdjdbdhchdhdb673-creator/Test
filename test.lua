-- GOTHBEACH ULTIMATE v4.0 [FULL FIX]
...
-- ============================================================
local Window = KavoLambo.CreateLib("GOTHBEACH ULTIMATE v4.0 [FULL FIX]")
...
local skyColor = Color3.fromRGB(135, 206, 235) -- голубой по умолчанию
local espObjects = {}
local glowObjects = {}
...
CombatSection:NewToggle("Aimbot (мгновенное наведение)", "", function(state)
    aimbotEnabled = state
end)
...
-- ============================================================
-- ВИЗУАЛЫ (РАБОЧИЙ ESP + ПОДСВЕТКА + ЦВЕТ НЕБА)
-- ============================================================
-- ФУНКЦИЯ ESP (имена над головами)
    ...
    for _, plr in pairs(Players:GetPlayers()) do
        if plr ~= player and plr.Character and plr.Character:FindFirstChild("Head") then
            ...
            if not head:FindFirstChild("ESP_Billboard") then
                ...
                label.BackgroundTransparency = 1
                label.Text = plr.Name .. " [" .. tostring(math.floor((root.Position - head.Position).Magnitude)) .. "m]"
...
VisualsSection:NewToggle("ESP (имена + дистанция)", "", function(state)
    espEnabled = state
    updateESP()
end)
...
VisualsSection:NewToggle("Подсветка игроков", "", function(state)
...
VisualsSection:NewColorPicker("Цвет подсветки", playerGlowColor, function(color)
    ...
    if playerGlowEnabled then updateGlow() end
end)
...
RunService.Stepped:Connect(function()
    ...
    -- Обновление ESP в реальном времени (дистанция)
    if espEnabled then
        for _, plr in pairs(Players:GetPlayers()) do
            if plr ~= player and plr.Character and plr.Character:FindFirstChild("Head") then
                local head = plr.Character.Head
                local bill = head:FindFirstChild("ESP_Billboard")
                if bill and bill:FindFirstChild("TextLabel") and root then
                    local dist = math.floor((root.Position - head.Position).Magnitude)
                    bill.TextLabel.Text = plr.Name .. " [" .. tostring(dist) .. "m]"
                end
            end
        end
    end
...
-- Обновление ESP и подсветки при появлении игроков
