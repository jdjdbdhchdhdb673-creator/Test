-- Загрузка Kavo UI через альтернативную рабочую ссылку (для Delta)
local Library = loadstring(game:HttpGet("https://pastebin.com"))()

-- Создание меню с красно-черной темой BloodSystem
local Window = Library.CreateLib("GOTHBEACH NINJA v2.0 [RAGE]", "BloodSystem")
...
-- ЭЛЕМЕНТЫ ВКЛАДКИ АВТОФАРМ
FarmSection:NewToggle("Авто-удары (Auto Swing)", "Сам качает Ниндзюцу с бешеной скоростью", function(state)
    autoSwing = state
end)

FarmSection:NewToggle("Авто-продажа (Auto Sell)", "Автоматически продает ниндзюцу, когда рюкзак полон", function(state)
    autoSell = state
end)

FarmSection:NewButton("Скорость х150 (Rage)", "Очень быстрый бег по карте", function()
